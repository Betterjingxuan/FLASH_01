...
public class VotingGame {
     ...
     * */
    public void voting_game(int[] given_weights) {
        //1）初始化给定的函数
        double halfSum = 1.0 * Arrays.stream(given_weights).sum() / 2;  //对given_weights中的数据求和
...
//        System.out.println(sum_weights);
     ...
     * 使用矩阵计算 */
    public void voting_game_2(int[] given_weights) {
        //1）初始化给定的函数
        double halfSum = 1.0 * Arrays.stream(given_weights).sum() / 2;  //对given_weights中的数据求和
//        System.out.println(sum);
//        ArrayList<Integer> all_weights = new ArrayList<>();
...
//        ShapMatrixEntry[] shap_matrix = ShapleyApproximate_voting_3(given_weights, halfSum);  //添加判断：矩阵是否为0

        double error_max = computeMaxError(shap_matrix, Info.voting_exact); //计算最大误差

        double error_ave = computeAverageError(shap_matrix, Info.voting_exact);  //计算平均误差
        System.out.println("My algo_2 voting Game:  " + "error_ave: " + error_ave + " \t" + "error_max: " + error_max );
        System.out.println("SortGrid time : " + (time_2 - time_1) * 0.001 );  //+ "S"
    }

    /*TODO [版本6+3] 不排序 + 子集名字不排序
     * 使用矩阵计算 */
    public void voting_game_3(int[] given_weights) {
        //1）初始化给定的函数
        double halfSum = 1.0 * Arrays.stream(given_weights).sum() / 2;  //对given_weights中的数据求和
        ...
        double error_max = computeMaxError(shap_matrix, Info.voting_exact); //计算最大误差
        double error_ave = computeAverageError(shap_matrix, Info.voting_exact);  //计算平均误差
        System.out.println("My algorithm_3 voting Game:  " + "error_ave: " + error_ave + " \t" + "error_max: " + error_max );
        System.out.println("Two NoSort time : " + (time_2 - time_1) * 0.001 );  //+ "S"
    ...
    private ShapMatrixEntry[] ShapleyApproximate_voting(int[] given_weights, double halfSum) {
...
//        computeLastLevel(grid_level, sum, given_weights, shap_matrix);
    ...
    /*TODO [版本2]：利用矩阵简化计算
     * 网格均匀划分 + 每层样本按比例分配 */
    private ShapMatrixEntry[] ShapleyApproximate_voting_2(int[] given_weights, double halfSum) {
...
//        computeLastLevel(grid_level, sum, given_weights, shap_matrix);
     ...
     * 网格均匀划分 + 每层样本按比例分配 */
    private ShapMatrixEntry[] ShapleyApproximate_voting_3(int[] given_weights, double halfSum) {
...
//        computeLastLevel(grid_level, sum, given_weights, shap_matrix);
    ...
    public void voting_evaluate_6(int[] given_weights) {
        //1）初始化给定的函数
        double halfSum = 1.0 * Arrays.stream(given_weights).sum() / 2;  //对given_weights中的数据求和
...
//        ShapMatrixEntry[] shap_matrix = ShapleyApproximate_voting_3(given_weights, halfSum);  //添加判断：矩阵是否为0
        ...
        double error_ave = computeAverageError(shap_matrix, Info.voting_exact);  //计算平均误差
        System.out.println("My alg_6 improve voting Game : " + "error_ave: " + error_ave + " \t" + "error_max: " + error_max );
        System.out.println("My alg_6(twoMat + noSort) time : " + (time_2 - time_1) * 0.001 );  //+ "S"
    }

    /*TODO [版本6]：利用矩阵简化计算 (使用两个矩阵) --> 网格不排序
     * 网格均匀划分 + 每层样本按比例分配 */
    private ShapMatrixEntry[] ShapleyApproximate_voting_6(int[] given_weights, double halfSum) {
...
//        computeLastLevel(grid_level, sum, given_weights, shap_matrix);
     ...
     * 2）无weight, 按照项的顺序分；
     * */
    private ShapMatrixEntry[] ShapleyApproximate_voting_x(int[] given_weights, double halfSum) {
...
//        computeLastLevel(grid_level, sum, given_weights, shap_matrix);
    ...
    private int[][] constructEvaluateMatrix(int[] given_weights, double halfSum) {
    ...
    private Grid computeSingleFeature_voting(ShapMatrixEntry[] shap_matrix, int[] given_weights, double halfSum) {
...
//            System.out.println(i + ": " + weight(0, Info.num_of_features) + " * " + value);
    ...
    private void computeMatrix_voting(ShapMatrixEntry[] shap_matrix, int[] given_weights, double halfSum, int[][] evaluateMatrix) {
    ...
    // 原版是 computeSingleFeature_voting()
    private Grid initialLevelCompute_voting(ShapMatrixEntry[] shap_matrix, int[] given_weights, double halfSum, int[][] evaluateMatrix) {
...
//        computeLevelTwo(twoCoalition_set);  //计算第二层（被减去的特征子集的长度为2）
    ...
    // 原版是 computeSingleFeature_voting()
    private Grid initialLevelCompute_voting_noSort(int[] given_weights, double halfSum, int[][] evaluateMatrix, int level_index, Allocation allo) {
    ...
    public ArrayList<FeatureSubset> randomSubsets(int m, int len, int[] given_weights, double halfSum){
    ...
    private Grid computeNextLevel_voting(int level, Grid grid, int[] given_weights, ShapMatrixEntry[] shap_matrix, Allocation all, double halfSum) {
...
//                        index++;
    ...
    private Grid computeNextLevel_voting_2(int level, Grid grid, int[] given_weights, ShapMatrixEntry[] shap_matrix, Allocation all, double halfSum) {
...
//                        index++;
    ...
    //原版是computeNextLevel_5()
    // weight[] 计算每层的采样权重； checkAndAllocate_transfer_4（）：按照每组个数等比例抽样；当前层生成的元素 list ：ArrayList();
    private Grid computeNextLevel_voting_3(int level, Grid grid, int[] given_weights, ShapMatrixEntry[] shap_matrix, Allocation all, double halfSum) {
...
//                        index++;
    ...
    //原版是computeNextLevel_5()
    // weight[] 计算每层的采样权重； checkAndAllocate_transfer_4（）：按照每组个数等比例抽样；当前层生成的元素 list ：ArrayList();
    private Grid computeNextLevel_voting_4(int level, Grid grid, int[] given_weights, ShapMatrixEntry[] shap_matrix, Allocation all, double halfSum) {
...
//                        index++;
    ...
    //TODO 构建矩阵存储连续的联盟值
    private int[][] constructLevelMatrix(int[] given_weights, double halfSum){
        int[][] matrix = new int[Info.num_of_features_voting][Info.num_of_features_voting];
        for(int i=0; i<Info.num_of_features_voting; i++) {   //i: 横坐标 (也是第1个item)
            ArrayList<Integer> subset = new ArrayList<>();
            subset.add(i);  //单个特征联盟，1-联盟
...
//            System.out.println(i + " : " + entry.sum + "\t  ");
    ...
    private int value_voting(ArrayList<Integer> subset, int[] given_weights, double halfSum) {
        int weights_sum = 0;
        //求weights_sum
        for(int ele : subset){
