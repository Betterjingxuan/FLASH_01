...
public class VotingGame_scale {
     ...
     * 使用矩阵计算 */
    public void voting_game_2() {
        try {
            /*【改】：改用given_weight*/
            String path = Info.ROOT + "voting_" + this.num_of_features + ".npy";
            DataInputStream dataInputStream = new DataInputStream(Files.newInputStream(Paths.get(path)));
...
//            double halfSum = 1.0 * Arrays.stream(given_weights).sum() / 2;  //对given_weights中的数据求和
    ...
    //        ShapMatrixEntry[] shap_matrix = ShapleyApproximate_voting_3(given_weights, halfSum);  //添加判断：矩阵是否为0
            ...
            System.out.println("voting Game:  " + "error_ave: " + error_ave + " \t" + "error_max: " + error_max );
    ...
    public void voting_game_6() {
        try {
            ...
            long time_1 = System.currentTimeMillis();
            shap_matrix = ShapleyApproximate_voting_6(shap_matrix, given_weights, halfSum);  //使用矩阵计算
            long time_2 = System.currentTimeMillis();
            ...
            double error_ave = computeAverageError(shap_matrix, this.benchmark_sv);  //计算平均误差
            System.out.println("voting Game_6:  " + "error_ave: " + error_ave + " \t" + "error_max: " + error_max );
            System.out.println("time : " + (time_2 - time_1) * 0.001 );  //+ "S"
    ...
    private ShapMatrixEntry[] ShapleyApproximate_voting(double[] given_weights, double halfSum) {
        ...
        allo.sampleAllocation_2(this.num_of_features-1); //sample_num_level 每一层采样几个
...
//        computeLastLevel(grid_level, sum, given_weights, shap_matrix);
     ...
     * 网格均匀划分 + 每层样本按比例分配 */
    private ShapMatrixEntry[] ShapleyApproximate_voting_6(ShapMatrixEntry[] shap_matrix, double[] given_weights, double halfSum) {
        ...
        //判断并记录从哪一层开始计算
        int level_index = checkLevel_2(levelMatrix);  //level_index：表示这层subset的length -1 (因为从0开始)
...
//            System.out.println(i + ": " + weight(0, Info.num_of_features) + " * " + value);
    ...
    //TODO 判断从哪层开始计算, 修改逻辑错误
    private int checkLevel_2(int[][] levelMatrix) {
        /* 记录开始计算的层数 */
        int line_ind = 0;
        //在每个对角线上检查
        int limit_step = Info.allConf;
        ...
        return Math.max(line_ind, limit_step);
    }
...
//            System.out.println(i + " : " + entry.sum + "\t  ");
    ...
    //TODO 输出和打印shapley value
    private void printShapleyValue_2(ShapMatrixEntry[] shap_matrix) {
        for(ShapMatrixEntry entry : shap_matrix){
            entry.sum = entry.sum / this.num_of_features;
//            System.out.println(i + " : " + entry.sum + "\t  ");
        }
    }
