...
import java.util.*;
import structure.*;
public class VotingGame_ablation {
     ...
     * */
    public void voting_evaluate_1(double[] given_weights) {
        //1）初始化给定的函数
...
//        System.out.println(sum_weights);
        ...
        System.out.println("voting Game (mining_6 + alloc_2) : " + "error_max: " + error_max + " \t" + "error_ave: " + error_ave );
        System.out.println("time : " + (time_2 - time_1) * 0.001 );  //+ "S"
    }

    /*TODO [版本2] 均匀划分 + 使用矩阵计算 */
    public void voting_evaluate_2(double[] given_weights) {
        //1）初始化给定的函数
        double halfSum = Arrays.stream(given_weights).sum() / 2;  //对given_weights中的数据求和
//        System.out.println(sum);
//        ArrayList<Integer> all_weights = new ArrayList<>();
...
//        ShapMatrixEntry[] shap_matrix = ShapleyApproximate_voting_3(given_weights, halfSum);  //添加判断：矩阵是否为0
        ...
        double error_ave = computeAverageError(shap_matrix, Info.voting_exact);  //计算平均误差
        System.out.println("voting Game (mining_6 + alloc_3 + Evaluate Matrix) : " + "error_ave: " + error_ave + " \t" + "error_max: " + error_max );
        System.out.println("time : " + (time_2 - time_1) * 0.001 );  //+ "S"
    }

    /*TODO [版本3] 非均匀划分 */
    public void voting_evaluate_3(double[] given_weights) {
        //1）初始化给定的函数
        double halfSum = Arrays.stream(given_weights).sum() / 2;  //对given_weights中的数据求和
//        System.out.println(sum);
//        ArrayList<Integer> all_weights = new ArrayList<>();
...
//        System.out.println(sum_weights);
        ...
        long time_1 = System.currentTimeMillis();
        ShapMatrixEntry[] shap_matrix = ShapleyApproximate_voting_3(given_weights, halfSum);  //均匀网格 + 比例分配每层样本
        long time_2 = System.currentTimeMillis();

        double error_max = computeMaxError(shap_matrix, Info.voting_exact); //计算最大误差

        double error_ave = computeAverageError(shap_matrix, Info.voting_exact);  //计算平均误差
        System.out.println("voting Game (mining_6 + 非均匀划分) : " + "error_max: " + error_max + " \t" + "error_ave: " + error_ave );
        System.out.println("time : " + (time_2 - time_1) * 0.001 );  //+ "S"
    }

    /*TODO  [版本4] 非均匀划分 + 使用矩阵计算 */
    public void voting_evaluate_4(double[] given_weights) {
        //1）初始化给定的函数
        double halfSum = Arrays.stream(given_weights).sum() / 2;  //对given_weights中的数据求和
//        System.out.println(sum);
//        ArrayList<Integer> all_weights = new ArrayList<>();
...
//        System.out.println(sum_weights);
        ...
        ShapMatrixEntry[] shap_matrix = ShapleyApproximate_voting_4(given_weights, halfSum);  //使用矩阵计算
...
//        ShapMatrixEntry[] shap_matrix = ShapleyApproximate_voting_3(given_weights, halfSum);  //添加判断：矩阵是否为0
        ...
        double error_ave = computeAverageError(shap_matrix, Info.voting_exact);  //计算平均误差
        System.out.println("voting Game (mining_6 + alloc_3 + 非均匀划分) : " + "error_ave: " + error_ave + " \t" + "error_max: " + error_max );
        System.out.println("time : " + (time_2 - time_1) * 0.001 );  //+ "S"
    }

    /*TODO [版本5] 均匀划分 + 使用2个矩阵计算 （版本2的升级版）*/
    public void voting_evaluate_5(double[] given_weights) {
        //1）初始化给定的函数
        double halfSum = Arrays.stream(given_weights).sum() / 2;  //对given_weights中的数据求和
//        System.out.println(sum);
//        ArrayList<Integer> all_weights = new ArrayList<>();
...
//        System.out.println(sum_weights);
        ...
        ShapMatrixEntry[] shap_matrix = ShapleyApproximate_voting_5(given_weights, halfSum);  //使用矩阵计算
...
//        ShapMatrixEntry[] shap_matrix = ShapleyApproximate_voting_3(given_weights, halfSum);  //添加判断：矩阵是否为0
        ...
        double error_ave = computeAverageError(shap_matrix, Info.voting_exact);  //计算平均误差
        System.out.println("voting Game(mining_6 + alloc_3 + twoMat) : " + "error_ave: " + error_ave + " \t" + "error_max: " + error_max );
        System.out.println("TwoMat improve time : " + (time_2 - time_1) * 0.001 );  //+ "S"
    }


    /*TODO [版本1]： 网格均匀划分 + 每层样本按比例分配
...
//        System.out.println("level initiall: " + grid_level.grids_row[0].get(0).name.size());
        ...
        for(int ind=1; ind <Info.num_of_features_voting; ind++){
            ...
            // 因为网格中不一定都有值的，可能网格中不存在元素： grid_level.grids_row[0].get(0).name.size() 可能空指针异常
            // 5) 计算某一层，存储到网格中
            grid_level = computeNextLevel_3(ind, grid_level, halfSum, given_weights, shap_matrix);  //这是均匀划分的方式
//            grid_level = computeNextLevel_voting(ind, grid_level, given_weights, shap_matrix, allo, halfSum);  //这是均匀划分 + 比例抽样(log降维)
            //computeNextLevel_2() 换成了结构体allocation[]！  某层中结构体不存在，就设置为0（因为值过高，用调节）
//            System.out.println("level : " + grid_level.grids_row[0].get(0).name.size());
//            System.out.print("level: " + ind + "\t");
...
//        computeLastLevel(grid_level, sum, given_weights, shap_matrix);
    ...
    /*TODO [版本4]：利用矩阵简化计算 + 非均匀网格
     * 网格均匀划分 + 每层样本按比例分配 */
    private ShapMatrixEntry[] ShapleyApproximate_voting_4(double[] given_weights, double halfSum) {
...
//            grid_level = computeNextLevel_3(ind, grid_level, sum, given_weights, shap_matrix);  //这是均匀划分的方式
//            grid_level = computeNextLevel_voting(ind, grid_level, given_weights, shap_matrix, allo, halfSum);  //这是均匀划分 + 比例抽样(log降维)
            grid_level = computeNextLevel_3(ind, grid_level, halfSum, given_weights, shap_matrix);  //去除list中重复的元素！
...
//        computeLastLevel(grid_level, sum, given_weights, shap_matrix);
    ...
    /*TODO [版本5]：利用矩阵简化计算 (使用两个矩阵)
     * 网格均匀划分 + 每层样本按比例分配 */
    private ShapMatrixEntry[] ShapleyApproximate_voting_5(double[] given_weights, double halfSum) {
...
//        allo.sampleAllocation_2(Info.num_of_features_voting-1); //sample_num_level 每一层采样几个 【逻辑错误】
        ...
        computeMatrix_voting_3(shap_matrix, evaluateMatrix, levelMatrix, level_index);    //grid_level: 每层的循环变量

        // 5)计算第2层（level = 被减去的特征的长度）
        Grid grid_level = initialLevelCompute_voting_2(given_weights, halfSum, evaluateMatrix, level_index, allo);  //level =2 （存储长度为2的特征子集）
...
//            System.out.println(i + ": " + weight(0, Info.num_of_features) + " * " + value);
    ...
    //TODO 【版本2】添加判断条件，判定矩阵是否全为0
    //（矩阵全为0，意味着这些2-联盟在拓展上没有区别，应该考虑对特征本身的权重做成网格来拓展）
    // 计算1-联盟 & 2-联盟的shapley值 => 利用矩阵EvaluateMatrix
    // 均匀划分：为单个特征计算边际贡献（所有单特征）复制版本3，用于voting_game
    // 原版是 computeSingleFeature_voting()
    private void computeMatrix_voting_2(ShapMatrixEntry[] shap_matrix, int[] given_weights, double halfSum, int[][] evaluateMatrix) {
...
//        computeLevelTwo(twoCoalition_set);  //计算第二层（被减去的特征子集的长度为2）
    ...
    //TODO 计算初始层，并存储到网格中(3-联盟-2联盟的值) => 利用矩阵EvaluateMatrix
    // 原版是 computeSingleFeature_voting()
    private Grid initialLevelCompute_voting_2(double[] given_weights, double halfSum, int[][] evaluateMatrix, int level_index, Allocation allo) {
        ...
        if(level_index > 2){
            ...
            //放入网格中
            grids.ConstructGrid_2(result);  //这是均匀划分的方式
        }
...
//            });
    ...
    // allocations[] 装进了结构体
    private Grid computeNextLevel_3(int level, Grid grid_1, double halfSum, double[] given_weights, ShapMatrixEntry[] shap_matrix) {
...
//                        index++;
        ...
        //4)这一层完成后，存储到结构体中
        //***********************************
        gridStructure.ConstructGrid(list);  //这是均匀划分的方式
