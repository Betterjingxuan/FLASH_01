...
public class CCN_algorithm_double_3 {
    ...
    //TODO 每次重新赋值
    public void CCN_Shap_eva(boolean gene_weight, String model){
...
//            ShapMatrixEntry[] shap_matrix = computeShapBySampling_3(game, initial_m, model, random);
            ShapMatrixEntry[] shap_matrix = computeShapBySampling_eva(game, this.initial_m, model, random, total_evaluateNum);
...
//        }
    ...
    public void CCN_Shap_eva_3(boolean gene_weight, String model){
...
//        }
    ...
    //TODO count --
    public void CCN_Shap_eva_2(boolean gene_weight, String model){
...
//        double ave_mse = 0;
        ...
        for(int i=0; i< Info.timesRepeat; i++) {
            ...
            long time_1 = System.currentTimeMillis();
//            ShapMatrixEntry[] shap_matrix = computeShapBySampling_3(game, initial_m, model, random);
            ShapMatrixEntry[] shap_matrix = computeShapBySampling_eva_2(game, this.initial_m, model, random);
            long time_2 = System.currentTimeMillis();
...
//        }
    ...
    //TODO 通过CCN algorithm 计算 shapley value (全改进后半截)
    private ShapMatrixEntry[] computeShapBySampling_eva(GameClass game, int initial_m, String model, Random random, int total_evaluateNum) {
...
//        this.num_samples = (int) (0.5 * this.num_samples - count);
        ...
        for (int j = (int) Math.ceil(this.num_features / 2.0) - 1; j < this.num_features; j++) {
            arr_m[j] = Math.max(0, (int) Math.ceil(total_evaluateNum * Math.sqrt(sigma_k[j] + sigma_n_k[j]) / var_sum)/2);
...
//                }
    ...
    private ShapMatrixEntry[] computeShapBySampling_eva_2(GameClass game, int initial_m, String model, Random random) {
...
//        this.num_samples = (int) (0.5 * this.num_samples - count);
        this.total_num_evaluations = this.total_num_evaluations - evaluations_num;
        int[] arr_m = new int[this.num_features];
        Arrays.fill(arr_m, 0); // Initialize m to zeros
        for (int j = (int) Math.ceil(this.num_features / 2.0) - 1; j < this.num_features; j++) {
            arr_m[j] = Math.max(0, (int) Math.ceil(this.total_num_evaluations * Math.sqrt(sigma_k[j] + sigma_n_k[j]) / var_sum)/2);
        }
...
//                }
    ...
    private ArrayList<Integer> permutation_2(ArrayList<Integer> idxs, Random PermutationGene) {
        //1) 初始化perm序列（initial 对应的位置是对应的值）
        ArrayList<Integer> perm = new ArrayList<>(idxs);
        ...
        // 随机打乱序列
        for(int i=1; i<idxs.size(); i++){  // (int i=numFeatures/2; i>1; i--)  (int i=0; i<numFeatures/2; i++)
            int j = random.nextInt(idxs.size()); //从[0, i+1)中随机选取一个int
