...
//TODO: [HashSet 提速版]重写函数，优化subset的去重函数，sort浪费时间 --> computeNextLevel_airport_4（）

public class Airport_1 {

    int num_of_features = Info.num_of_features_airport;

    /*TODO [Airport_game]
     * given_weights: 给定的weights[] 用于计算value_function*/
    public void airport_game(int[] given_weights) {
...
//        ShapMatrixEntry[] shap_matrix = ShapleyApproximate_airport_2(given_weights); //使用矩阵计算(复制voting Game,改成airport)
    ...
    /*TODO [版本2] 不排序放入网格 */
    public void airport_game_3(int[] given_weights) {
...
//
    ...
    //TODO [HashSet 提速版] 复制MiningShap_6 + computeNextLevel_airport_4
    private ShapMatrixEntry[] ShapleyApproximate_airport_1(int[] given_weights) {
...
//        allo.sampleAllocation_2(this.num_of_features);
        ...
        Grid grid_level = initialLevelCompute_airport_4(shap_matrix, given_weights, evaluateMatrix);  //HashSet 提速版 //level =2 （存储长度为2的特征子集）

        //4)依次生成联盟 (ind / level 等于被减去的联盟的长度)
        for(int ind=2; ind <this.num_of_features; ind++){
            grid_level = computeNextLevel_airport_4(ind, grid_level, given_weights, shap_matrix, allo);  //HashSet 提速版
        }
    ...
    private ShapMatrixEntry[] ShapleyApproximate_airport_2(int[] given_weights) {
...
//        allo.sampleAllocation_2(Info.num_of_features_voting-1); //sample_num_level 每一层采样几个 【逻辑错误】
        ...
        // 5)计算第2层（level = 被减去的特征的长度）
        Grid grid_level = initialLevelCompute_airport_4(shap_matrix, given_weights, evaluateMatrix);  //HashSet 提速版
    ...
    private ShapMatrixEntry[] ShapleyApproximate_airport_3(int[] given_weights) {
        //1) 用一个大数组记录特征对应的shapley value之和
        ShapMatrixEntry[] shap_matrix = new ShapMatrixEntry[this.num_of_features];
        for(int i=0; i<shap_matrix.length; i++){
            shap_matrix[i] = new ShapMatrixEntry();
...
//        allo.sampleAllocation_2(this.num_of_features);
        ...
        // 5)计算第2层（level = 被减去的特征的长度）
        Grid grid_level = initialLevelCompute_airport_4(shap_matrix, given_weights, evaluateMatrix);  //level =2 （存储长度为2的特征子集）

        //4)依次生成联盟 (ind / level 等于被减去的联盟的长度)
        for(int ind=2; ind <this.num_of_features; ind++){
            grid_level = computeNextLevel_airport_4(ind, grid_level, given_weights, shap_matrix, allo);  //去除list中重复的元素！
        }
    ...
    private int[][] constructEvaluateMatrix(int[] given_weights) {
    ...
    private void computeMatrix_airport(ShapMatrixEntry[] shap_matrix, int[] given_weights, int[][] evaluateMatrix) {
    ...
    // 原版是 computeSingleFeature_voting()
    private Grid initialLevelCompute_airport_4(ShapMatrixEntry[] shap_matrix, int[] given_weights, int[][] evaluateMatrix) {

        //性质：矩阵是一个对称的矩阵，所以2-联盟只需要看一半，存一半就可以

        // 1）初始化网格结构
        Grid grids = new Grid();  //存储2-联盟的网格
        HashSet<FeatureSubset> twoCoalition_set = new HashSet<>();  //用list存储所有的2-项集
        //数组：虽然定长，但删除不方便
...
//        });
        ...
        grids.ConstructGrid_4(twoCoalition_set);  //这是均匀划分的方式
        return grids;
    }

    //TODO [第2版] 不排序放入网格
    private Grid initialLevelCompute_airport_2(ShapMatrixEntry[] shap_matrix, int[] given_weights, int[][] evaluateMatrix) {
...
//        });
    ...
    private Grid computeNextLevel_airport_4(int level, Grid grid, int[] given_weights, ShapMatrixEntry[] shap_matrix, Allocation all) {
...
//        ArrayList<FeatureSubset> list = new ArrayList<>();
        ...
        //【添加步骤】list中去除重复元素
        HashSet<FeatureSubset> setWithoutDuplicates = new HashSet<>();
...
//        System.out.println();
        ...
        for(int ind=0; ind<all.allocations.length; ind++){  //分配方案[]：给每个网格分配的个数
            ...
            for(int counter=0; counter < sample_num; counter ++){  //count是采样的计数器
                ...
                for(int i=0; i<this.num_of_features; i++){
                    HashSet<Integer> hashName = new HashSet<>(random_sample.hashName);
                    hashName.add(i);
                    if(random_sample.hashName.size() != hashName.size()){   //挑选出的random_sample不包含当前的特征，就可以构成新的
                        double value = value_airport_3(hashName, given_weights);
                        FeatureSubset newFeaSub = new FeatureSubset(hashName, value);
...
//                        }
        ...
        //4)这一层完成后，存储到结构体中
        //***********************************
        gridStructure.ConstructGrid_5(setWithoutDuplicates);  //HashSet + 不排序放入网格
        long time_2 = System.currentTimeMillis();
        System.out.println("ConstructGrid_5: " + (time_2 - time_1));
    ...
    //原版是computeNextLevel_5()
    // weight[] 计算每层的采样权重； checkAndAllocate_transfer_4（）：按照每组个数等比例抽样；当前层生成的元素 list ：ArrayList();
    private Grid computeNextLevel_airport_3(int level, Grid grid, int[] given_weights, ShapMatrixEntry[] shap_matrix, Allocation all) {
...
//            i++;
    ...
    private int value_airport_3(HashSet<Integer> subset, int[] given_weights) {
        int maxValue = 0;
        for(int ele : subset){
    ...
    //TODO 因为scale时生成的weight无序，所以不能直接用下标判断对应的返回值
    private int value_airport_2(ArrayList<Integer> subset, int[] given_weights) {
        int maxValue = 0;
        for(int ele : subset){
            maxValue = Math.max(given_weights[ele], maxValue);
        }
