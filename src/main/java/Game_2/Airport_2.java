...
//        allo.sampleAllocation_2(Info.num_of_features_voting-1); //sample_num_level 每一层采样几个 【逻辑错误】
        ...
        // 5)计算第2层（level = 被减去的特征的长度）
        Grid grid_level = initialLevelCompute_airport_2(shap_matrix, given_weights, evaluateMatrix);  //HashSet 提速版
...
//        ArrayList<FeatureSubset> list = new ArrayList<>();
        ...
        HashSet<HashSet<Integer>> nameSet = new HashSet<>();  //HashSet去除重复
...
//                    if(random_sample.hashName.size() != hashName.size()){   //挑选出的random_sample不包含当前的特征，就可以构成新的
                        ...
                        name.add(i);
                        HashSet<Integer> hashName = new HashSet<>(name);
...
//                        Collections.sort(newFeaSub.name);  //把元素的名字按顺序整理排好
                        if(!nameSet.contains(newFeaSub.hashName)){
                            nameSet.add(hashName);
...
//        long time_1 = System.currentTimeMillis();
        //4)这一层完成后，存储到结构体中
        //***********************************
        gridStructure.ConstructGrid_3(setWithoutDuplicates);  //HashSet + 不排序放入网格
//        long time_2 = System.currentTimeMillis();
//        System.out.println("ConstructGrid_2/3: " + (time_2 - time_1));
